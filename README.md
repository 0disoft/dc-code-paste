...
Playwright CI는 Chromium, Firefox, WebKit에서 편집·복사 흐름을 검사한다. 실제 OS 클립보드와 DCInside 에디터의 붙여넣기 결과는 별도로 확인해야 한다. 자동 복사가 거절되면 화면에 표시된 HTML 원문을 복사해 DCInside **HTML 모드**에 붙여넣는다. 일반 에디터에 HTML 원문을 붙여넣으면 서식이 적용되지 않는다.
